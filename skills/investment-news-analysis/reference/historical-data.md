...
日报采用“精简可审计版”后，优先提取以下章节：

1. `一、核心指标速览与横向对比`：完整核心持仓表、份额变化和组合结论。
2. `五、复盘与风险雷达`：上一轮判断的验证状态、风险变化和动作边界。
3. `六、逐基金状态复核与操作建议`：12 行状态快照、成本锚定触发线和置信度。
4. `七、今日关注要点`：下一轮最优先验证的未决事项。
5. `八、数据附录`：raw data、新闻归档、最近基准、HTML 路径、缺口和反向证据。

当需要某基金完整的“判断理由 / 事实依据 / 信息边界”，从第八章的同日 HTML 路径打开 HTML 第四章决策卡；不得把 HTML 当作净值、份额或新闻事实的原始来源，仍应回溯 `raw_data/` 和 `item_summaries/`。

...
本 Skill 中所有 `scripts/xxx.py` 和 `reference/xxx.md` 路径均**相对于 Skill 目录**（即 `skill_view` 返回的 `skill_dir`）。执行脚本或读取参考文件时，必须使用 skill 目录的绝对路径作为前缀。

**示例**：若 `skill_dir` 为 `/path/to/fund-deep-research`，则：
- `scripts/precheck.py` → 执行 `/path/to/fund-deep-research/scripts/precheck.py`
- `reference/report-template.md` → 读取 `/path/to/fund-deep-research/reference/report-template.md`

脚本内部的 `web-platform/` 和 `基金研究报告/` 路径相对于**当前工作目录**（workspace），脚本已通过 `os.getcwd()` 自动解析，无需额外处理。
...
```
python scripts/precheck.py <基金代码>
```
...
python scripts/parallel_data_collection_v2.py ${CODE}
```
...
```
python scripts/check_data_integrity.py <基金代码>
```
...
> **Step 6 开始写报告前**，必须先执行：
> ```
> read_file reference/report-template.md
> ```
...
> **写第二章前**，必须先执行：
> ```
> read_file reference/scoring-matrix.md
> ```
...
> **报告完成提交前**，必须先执行：
> ```
> read_file reference/checklist-and-faq.md
> ```
...
```bash
python3 scripts/build_json_from_cache.py <基金代码>
```
...
```bash
# 仅填充缺失字段（默认）
python3 scripts/merge_b_fields.py <基金代码> /tmp/fund_research_<代码>/b_fields.json

# 强制更新已有字段
python3 scripts/merge_b_fields.py <基金代码> /tmp/fund_research_<代码>/b_fields.json --overwrite
```
...
```bash
# 校验单基金 JSON 结构
python3 scripts/validate_json_schema.py <基金代码>
...
# Step 1：数据采集（已有）
python3 scripts/parallel_data_collection_v2.py ${CODE}

# Step 2：生成研究报告（AI 写作，参照 SKILL.md Step 6）

# Step 3：A 类字段自动写入
python3 scripts/build_json_from_cache.py ${CODE}
...
# Step 5：B 类字段合并
python3 scripts/merge_b_fields.py ${CODE} /tmp/fund_research_${CODE}/b_fields.json

# Step 6：【新增】验证 JSON 格式是否符合前端期望
python3 scripts/validate_json_schema.py ${CODE}

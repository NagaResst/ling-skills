...
#### 第二层：内容质量检查
- [ ] **拐点三线完整**：3.2 节中 🌐外部环境 和 📊归因评价 出现次数均等于 `inflection_points.json` 的拐点总数；👤经理操作 行不得全缺（被动指数基金除外，可写"被动指数跟踪，无主动操作"）
- [ ] **无占位符残留**：报告中不出现"数据缺失"、"暂无公开数据"、"历史行业穿透数据缺失"等占位文本（若确实无法获取数据，应写明具体原因和已尝试的搜索路径）
- [ ] **4.4 言行一致性审计**：含至少 2 个季度的季报原文引用（带 `[Step5-QuarterlyQuote]` 来源），每条对照实际持仓给出 ✅⚠️❌ 判断
- [ ] **4.5 经理变更历史**：有结构化表格（非一行纯文字），含经理名、任期起止、管理时长
- [ ] **6.3 持仓演变表**：无"历史行业穿透数据缺失"占位文本；如无法可靠归类，基于标的主营业务人工归类并注明"基于标的主营业务归类"
- [ ] **10.4 持仓回顾**：有具体 NAV 数值（加仓区间、减仓区间、硬止损线），非纯文字描述
- [ ] **8.4 三情景分析**：悲观/基准/乐观三行完整，每行含对应触发条件和预期净值变动
